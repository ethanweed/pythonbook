...
# Not a lot of surprises here. But there's a few things worth talking about, even with such a simple example. Firstly, it's important that you understand how to read the extract. In this example, what *I* typed was the `10 + 20` part. Obviously, the correct answer to the sum `10 + 20` is `30`, and not surprisingly Python has printed that out as part of its response. But if you are using Jupyter Notebooks, you will probably also see something like `In [1]` and `Out [1]` part, which probably doesn't make a lot of sense to you right now. You're going to see that a lot. You can think of `Out [1]: 30` as if Python were saying "the answer to the 1st question you asked is 30". That's not quite the truth, but it's close enough for now. If you see `In [*]`, that means that Python is still working on whatever you asked it to do.
