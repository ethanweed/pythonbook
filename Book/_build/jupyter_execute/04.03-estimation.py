...
# 
# > *For even the most stupid of men, by some instinct of nature, by himself and without any instruction (which is a remarkable thing), is convinced that the more observations have been made, the less danger there is of wandering from one's goal* {cite}`Stigler1986`
# 
...
import pandas as pd
df = pd.DataFrame(
    {'Person 1': np.random.normal(loc=100,scale=15,size=5).astype(int),
     'Person 2': np.random.normal(loc=100,scale=15,size=5).astype(int),
     'Person 3': np.random.normal(loc=100,scale=15,size=5).astype(int),
     'Person 4': np.random.normal(loc=100,scale=15,size=5).astype(int),
     'Person 5': np.random.normal(loc=100,scale=15,size=5).astype(int),
     'Sample Mean': 
    }) 
